...
import type { GitLabSetupResponse, GitLabStatusResponse } from "@/lib/relay-client"
...
export default function DashboardPage() {
  ...
  const [gitlabStatus, setGitlabStatus] = useState<GitLabStatusResponse | null>(null)
  ...
  const handleGitlabConnect = async (data: GitLabSetupResponse) => {
    console.log('GitLab connected:', data.integration_id, `${data.projects.length} projects`)
    setGitlabError(null)
    
    try {
      const status = await relayClient.gitlab.status()
      setGitlabStatus(status)
      setGitlabConnected(status.connected && (status.status?.synced || status.repos_count > 0))
    } catch {
  ...
  const handleGitlabRefresh = async () => {
    ...
    setGitlabError(null)
    try {
      await relayClient.gitlab.refresh()
      const status = await relayClient.gitlab.status()
      setGitlabStatus(status)
      setGitlabConnected(status.connected && (status.status?.synced || status.repos_count > 0))
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to refresh GitLab sync'
    ...
    <div className="content-spacing max-w-2xl mx-auto">
      ...
      <div className={cn(
        ...
        <Card className="bg-card/50 shadow-none border-border/60 p-2 gap-1">
          ...
          {!gitlabError && gitlabStatus?.status?.errors && gitlabStatus.status.errors.length > 0 && (
            <div className="flex items-start gap-3 px-3 py-2.5 rounded-md bg-amber-500/10 text-amber-600 dark:text-amber-500 text-sm">
              ...
              <div>
                ...
                <span className="ml-1">
                  {gitlabStatus.status.errors.length} {gitlabStatus.status.errors.length === 1 ? 'project' : 'projects'} failed to sync.
                </span>
              </div>
            </div>
          )}
