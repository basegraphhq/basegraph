...
function formatErrorMessage(error: string, status: number): string {
	...
	if (
		error.includes("validating token") ||
		error.includes("401") ||
		status === 401
	) {
		...
		return (
			error ||
			"Invalid token. Please check your Personal Access Token and try again."
		);
	}
	if (status === 403) {
		return (
			error ||
			"Token does not have sufficient permissions. Ensure the token has 'api' scope and the user has Maintainer access to at least one project."
		);
	}
